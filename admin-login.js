...
export default async function handler(req, res) {
  if (req.method === "POST") {
    ...
    const packs = isUnitPricing ? Math.max(1, Math.floor(Number(qty) || 1)) : 1;
    const totalPrice = isUnitPricing ? Math.round(Number(item.price) * packs * 100) / 100 : Number(item.price);
